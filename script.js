// ===== شاشة التحميل =====
    ...
    // ===== حركة الختم المائي عند التمرير (parallax خفيف) =====
    const seal = document.getElementById('watermark-seal');
    const systemPrefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    let a11yMotionDisabled = false;
    function reduceMotion(){ return systemPrefersReducedMotion || a11yMotionDisabled; }
      ...
      window.addEventListener('scroll', () => {
        ...
        if (!ticking) {
          window.requestAnimationFrame(() => {
            ...
            const rotate = window.scrollY * 0.015;
            seal.style.transform = `translateY(${offset}px) rotate(${rotate}deg)`;
            ticking = false;
          });
          ticking = true;
        }
    ...
    const A11Y_STORAGE_KEY = 'khadamati-a11y';
    const a11yToggle = document.getElementById('a11y-toggle');
    const a11yModal = document.getElementById('a11y-modal');
    const a11yModalClose = document.getElementById('a11y-modal-close');
    const a11yModalBackdrop = document.getElementById('a11y-modal-backdrop');
    const a11yFontValue = document.getElementById('a11y-font-value');
    const a11yFontDecrease = document.getElementById('a11y-font-decrease');
    const a11yFontIncrease = document.getElementById('a11y-font-increase');
    const a11yContrastToggle = document.getElementById('a11y-contrast-toggle');
    const a11yMotionToggle = document.getElementById('a11y-motion-toggle');
    const a11yResetBtn = document.getElementById('a11y-reset-btn');

    let a11yFontScale = 1;
    let a11yContrastEnabled = false;
    // a11yMotionDisabled معرّف مسبقًا أعلى الملف ويُسخدم هنا أيضًا

    function applyFontScale(){
      const percent = Math.round(a11yFontScale * 100);
      document.documentElement.style.zoom = percent + '%';
      a11yFontValue.textContent = percent + '%';
    }

    function applyContrast(enabled){
      a11yContrastEnabled = enabled;
      document.documentElement.classList.toggle('a11y-contrast', enabled);
      a11yContrastToggle.setAttribute('aria-checked', String(enabled));
    }

    function applyMotion(disabled){
      a11yMotionDisabled = disabled;
      document.documentElement.classList.toggle('a11y-reduce-motion', disabled);
      a11yMotionToggle.setAttribute('aria-checked', String(disabled));
    }
    ...
    function saveA11ySettings(){
      try {
        localStorage.setItem(A11Y_STORAGE_KEY, JSON.stringify({
          ...
          contrast: a11yContrastEnabled,
          motion: a11yMotionDisabled
        }));
      } catch (err) {
        // تجاهل بأمان إذا كان التخزين غير متاح
      }
    ...
    function loadA11ySettings(){
      try {
        ...
        if (typeof saved.fontScale === 'number') a11yFontScale = saved.fontScale;
        if (typeof saved.contrast === 'boolean') applyContrast(saved.contrast);
        if (typeof saved.motion === 'boolean') applyMotion(saved.motion);
      } catch (err) {
        // تجاهل بأمان إذا كانت البيانات المحفوظة غير صالحة
      }
      applyFontScale();
    }
    ...
    a11yToggle.addEventListener('click', openA11yModal);
    a11yModalClose.addEventListener('click', closeA11yModal);
    a11yModalBackdrop.addEventListener('click', closeA11yModal);
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape' && a11yModal.classList.contains('show')) closeA11yModal();
    });

    a11yFontDecrease.addEventListener('click', () => {
      a11yFontScale = Math.max(0.8, Math.round((a11yFontScale - 0.1) * 10) / 10);
      applyFontScale();
      saveA11ySettings();
    });
    a11yFontIncrease.addEventListener('click', () => {
      a11yFontScale = Math.min(1.4, Math.round((a11yFontScale + 0.1) * 10) / 10);
      applyFontScale();
      saveA11ySettings();
    });

    a11yContrastToggle.addEventListener('click', () => {
      applyContrast(!a11yContrastEnabled);
      saveA11ySettings();
    });

    a11yMotionToggle.addEventListener('click', () => {
      applyMotion(!a11yMotionDisabled);
      saveA11ySettings();
    });
    ...
    const translations = {
      ...
      a11yToggleAria: { ar: "إعدادات إمكانية الوصول", en: "Accessibility settings" },
      a11yTitle: { ar: "إعدادات إمكانية الوصول", en: "Accessibility settings" },
      a11yCloseAria: { ar: "إغلاق", en: "Close" },
      a11yFontSize: { ar: "حجم الخط", en: "Font size" },
      a11yFontDecreaseAria: { ar: "تصغير حجم الخط", en: "Decrease font size" },
      a11yFontIncreaseAria: { ar: "تكبير حجم الخط", en: "Increase font size" },
      a11yHighContrast: { ar: "وضع التباين العالي", en: "High contrast mode" },
      a11yReduceMotion: { ar: "تعطيل الحركات والانتقالات", en: "Disable animations & transitions" },
      a11yReset: { ar: "إعادة الضبط الافتراضي", en: "Reset to default" },
